...
if npx cdk deploy --require-approval never; then
    ...
    echo ""
    print_warning "Note: It may take 10-15 minutes for the service to fully start"
    print_warning "RDS database initialization and Airflow setup takes time"
    print_warning "Check ECS service status and CloudWatch logs if UI is not accessible"
    echo ""
