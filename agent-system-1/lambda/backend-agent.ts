...
const SYSTEM_PROMPT = `You are a Senior Backend Engineer AI agent.
Given a backend task, provide a concise technical implementation plan covering:
...
export const handler = async (event: SQSEvent) => {
  for (const record of event.Records) {
    ...
    await ddb.send(new PutCommand({
      ...
      Item: { taskId, agentRole: 'backend', task, output, completedAt: new Date().toISOString() },
    }));

    console.log(`[Backend Eng] taskId=${taskId} done`);
  }
};
